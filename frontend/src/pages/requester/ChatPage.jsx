...
export default function RequesterChatPage() {
  ...
  useEffect(() => {
    if (!chat) return;
    if (chat.status === "closed" || chat.status === "closing") {
  ...
  useEffect(() => {
    if (thumbRating) setTimeout(() => bottomRef.current?.scrollIntoView({ behavior: "smooth" }), 50);
  }, [thumbRating]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json();
        setSendError("Waiting for a responder to claim your request before you can send messages.");
        return;
      }
      setMessage("");
      fetchChat();
  ...
  const allEvents = [
    ...(initialRequestEvent ? [initialRequestEvent] : []),
  ...
  return (
    <Box sx={{ maxWidth: 720, mx: "auto", display: "flex", flexDirection: "column", height: "calc(100vh - 160px)" }}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, pb: 2, mb: 1, borderBottom: `0.5px solid ${borderColor}22`, flexWrap: "wrap" }}>
        ...
        {!isClosed && (
          <Button onClick={() => setTokenDialog(true)} startIcon={<TokenIcon sx={{ fontSize: "13px !important" }} />}
            sx={{ borderRadius: "99px", px: 2, py: 0.5, fontWeight: 700, fontSize: "0.72rem", textTransform: "none", border: `0.5px solid ${borderColor}33`, color: mutedColor, "&:hover": { bgcolor: `${borderColor}0d`, color: borderColor, borderColor: `${borderColor}66` } }}>
            Add tokens
          </Button>
        )}
        {!isClosed && (
          <Tooltip title="Close chat">
            <IconButton onClick={() => setCloseDialog(true)} size="small"
      ...
      <Box sx={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 0.75, pb: 2, "&::-webkit-scrollbar": { width: 4 }, "&::-webkit-scrollbar-thumb": { bgcolor: `${borderColor}22`, borderRadius: 2 } }}>
        {responderName && <SystemPill>{responderName} is answering your request.</SystemPill>}

        {timeline.map((item, i) => {
          if (item.type === "divider") return <DateDivider key={`d${i}`} label={item.label} borderColor={borderColor} mutedColor={mutedColor} />;
          if (item._type === "token_event") return <SystemPill key={item.requestID}>+{item.tokensSpent} tokens added · {formatTime(item._ts)}</SystemPill>;
          const isMe = item.senderType === "requester";
          return <ChatBubble key={item.messageID} isMe={isMe} text={item.message} time={formatTime(item.createdAt)} primary={primary} borderColor={borderColor} mutedColor={mutedColor} isDark={isDark} theme={theme} />;
        })}

        {isClosed && responderName && !ratingSubmitted && showRating && (
          <Box sx={{ mt: 2, display: "flex", flexDirection: "column", alignItems: "center", gap: 2, py: 3, px: 2, borderTop: `0.5px solid ${borderColor}15` }}>
            <SystemPill>{responderName} has marked this conversation as complete.{"\n"}Did they answer your query?</SystemPill>
            <Box sx={{ display: "flex", gap: 1.5 }}>
              {["No", "Yes"].map((val) => {
                const selected = thumbRating === val.toLowerCase();
                return (
                  <Button key={val} onClick={() => setThumbRating(val.toLowerCase())}
                    sx={{ borderRadius: "99px", px: 4, py: 1, fontWeight: 700, fontSize: "0.9rem", textTransform: "none", minWidth: 100, border: `1.5px solid ${selected ? borderColor : `${borderColor}28`}`, bgcolor: selected ? theme.palette.text.primary : "transparent", color: selected ? theme.palette.background.default : mutedColor, transition: animationsEnabled ? "all 0.15s ease" : "none", "&:hover": { bgcolor: selected ? theme.palette.text.primary : `${borderColor}0d`, color: selected ? theme.palette.background.default : borderColor } }}>
                    {val}
                  </Button>
                );
              })}
            ...
            {thumbRating && (
              <>
                ...
                <Box sx={{ display: "flex", gap: 0.25 }}>
                  {[1, 2, 3, 4, 5].map((s) => (
                    <IconButton key={s} onMouseEnter={() => setHoverStar(s)} onMouseLeave={() => setHoverStar(0)} onClick={() => setStarRating(s)}
                      sx={{ p: 0.5, color: s <= (hoverStar || starRating) ? "#f5a623" : `${borderColor}28`, transition: "color 0.1s ease" }}>
                      {s <= (hoverStar || starRating) ? <StarIcon sx={{ fontSize: 32 }} /> : <StarBorderIcon sx={{ fontSize: 32 }} />}
                    </IconButton>
                  ))}
                </Box>
        ...
        {isClosed && ratingSubmitted && <SystemPill>Thank you for your rating!</SystemPill>}
        {isClosed && !responderName && <SystemPill>This conversation is archived. You cannot respond to it.</SystemPill>}
        <div ref={bottomRef} />
      </Box>
      ...
      {!isClosed ? (
        <Box sx={{ display: "flex", gap: 1, alignItems: "flex-end", pt: 2, borderTop: `0.5px solid ${borderColor}22` }}>
          ...
          <IconButton onClick={handleSend} disabled={sending || !message.trim()}
            sx={{ width: 44, height: 44, borderRadius: "12px", flexShrink: 0, bgcolor: primary, color: "#fff", transition: animationsEnabled ? "all 0.15s ease" : "none", "&:hover": { filter: "brightness(0.9)", transform: animationsEnabled ? "scale(1.05)" : "none" }, "&.Mui-disabled": { opacity: 0.3, bgcolor: primary, color: "#fff" } }}>
      ...
      {/* Add Tokens Dialog */}
      <Dialog open={tokenDialog} onClose={() => setTokenDialog(false)} PaperProps={{ sx: { borderRadius: "12px", border: `0.5px solid ${borderColor}33`, bgcolor: theme.palette.background.paper, p: 1, minWidth: 340 } }}>
        <DialogTitle sx={{ fontWeight: 900, letterSpacing: "-0.5px" }}>Add Tokens</DialogTitle>
        <DialogContent sx={{ display: "flex", flexDirection: "column", gap: 2, pt: "8px !important" }}>
          <DialogContentText sx={{ color: mutedColor, fontSize: "0.85rem" }}>Add a new request with additional tokens to increase priority or expand scope.</DialogContentText>
          <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
            {TOKEN_PRESETS.map((t) => {
              const selected = isTokenPreset && tokenAmount === t;
              return (
                <Box key={t} component="button" onClick={() => { setTokenAmount(t); setCustomToken(""); }}
                  sx={{ height: 30, px: 2, borderRadius: "99px", border: `1.5px solid ${selected ? borderColor : `${borderColor}28`}`, bgcolor: selected ? theme.palette.text.primary : "transparent", color: selected ? theme.palette.background.default : mutedColor, fontWeight: 700, fontSize: "0.8rem", cursor: "pointer", fontFamily: theme.typography.fontFamily, "&:hover": { border: `1.5px solid ${borderColor}77` } }}>
                  {t}
                </Box>
              );
            })}
            <Box sx={{ height: 30, px: 1, borderRadius: "99px", width: 70, border: `1.5px solid ${!isTokenPreset && customToken ? borderColor : `${borderColor}28`}`, bgcolor: !isTokenPreset && customToken ? theme.palette.text.primary : "transparent", display: "flex", alignItems: "center" }}>
              <input value={customToken} onChange={(e) => { const raw = e.target.value; if (raw !== "" && !/^\d+$/.test(raw)) return; setCustomToken(raw); const v = parseInt(raw, 10); if (!isNaN(v) && v > 0) setTokenAmount(v); }} placeholder="custom" inputMode="numeric"
                style={{ width: "100%", border: "none", outline: "none", background: "transparent", textAlign: "center", fontWeight: 700, fontSize: "0.75rem", fontFamily: "inherit", color: !isTokenPreset && customToken ? theme.palette.background.default : theme.palette.text.secondary }} />
            </Box>
          </Box>
          <TextField label="What do you need help with?" value={tokenText} onChange={(e) => setTokenText(e.target.value)} multiline minRows={3} fullWidth
        ...
        <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
          <Button onClick={() => setTokenDialog(false)} sx={{ borderRadius: "99px", px: 3, fontWeight: 600, fontSize: "0.8rem", textTransform: "none", color: mutedColor, border: `0.5px solid ${borderColor}33`, "&:hover": { bgcolor: `${borderColor}08` } }}>Cancel</Button>
          <Button onClick={handleAddTokens} disabled={addingTokens || !tokenText.trim()}
      ...
      {/* Close Chat Dialog */}
      <Dialog open={closeDialog} onClose={() => setCloseDialog(false)} PaperProps={{ sx: { borderRadius: "12px", border: `0.5px solid ${borderColor}33`, bgcolor: theme.palette.background.paper, p: 1 } }}>
        <DialogTitle sx={{ fontWeight: 900 }}>Close this chat?</DialogTitle>
        <DialogContent>
          <DialogContentText sx={{ color: mutedColor, fontSize: "0.9rem" }}>The responder will no longer be able to reply. This cannot be undone.</DialogContentText>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
          <Button onClick={() => setCloseDialog(false)} sx={{ borderRadius: "99px", px: 3, fontWeight: 600, fontSize: "0.8rem", textTransform: "none", color: mutedColor, border: `0.5px solid ${borderColor}33`, "&:hover": { bgcolor: `${borderColor}08` } }}>Cancel</Button>
          <Button onClick={handleCloseChat} sx={{ borderRadius: "99px", px: 3, fontWeight: 600, fontSize: "0.8rem", textTransform: "none", bgcolor: theme.palette.error.main, color: "#fff", "&:hover": { bgcolor: theme.palette.error.dark } }}>
  ...
  return (
    <Box sx={{ display: "flex", justifyContent: isMe ? "flex-end" : "flex-start", px: 1 }}>
      <Box sx={{ maxWidth: "72%", display: "flex", flexDirection: "column", alignItems: isMe ? "flex-end" : "flex-start", gap: 0.25 }}>
        <Box sx={{ px: 2, py: 1.25, borderRadius: isMe ? "18px 18px 4px 18px" : "18px 18px 18px 4px", bgcolor: isMe ? primary : (isDark ? "#2a2a2a" : "#f0f0f0"), color: isMe ? "#fff" : theme.palette.text.primary, fontSize: "0.9rem", lineHeight: 1.55, wordBreak: "break-word" }}>
          {text}
        </Box>
        <Typography sx={{ fontSize: "0.6rem", color: mutedColor, opacity: 0.65, px: 0.5 }}>{time}</Typography>
      </Box>
    </Box>
  );
}
