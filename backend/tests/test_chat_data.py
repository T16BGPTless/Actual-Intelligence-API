...
from app.chat_data import api_ts, categories_from_flask_arg, profile_map, chat_summary_dict, message_dict, get_chat_or_none, create_chat_with_initial_request, build_chat_detail
...
def test_get_chat_or_none(monkeypatch):
    class FakeQ:
        ...
        def execute(self): return SimpleNamespace(data={"chat_id": "123"})
...
def test_create_chat_with_initial_request_success():
    ...
    class FakeClient:
        ...
        def update(self, d): return FakeUpd()
        
    res, err = create_chat_with_initial_request(FakeClient(), {"tokensToSpend": 5, "requestText": "Test"})
    assert not err
