...
def _patch_auth(monkeypatch):
    ...
    monkeypatch.setattr(
        res_routes, "service_client", lambda *a: SimpleNamespace()
    )  # Add this!
...
def test_browse_chats(client, monkeypatch):
    ...
    class FakeQ:
        ...
        def execute(self):
            return SimpleNamespace(data=[{"chat_id": "2", "status": "open"}])

    monkeypatch.setattr(
        res_routes, "user_client", lambda *a: SimpleNamespace(table=lambda *a: FakeQ())
    )
...
def test_claim_chat_success(client, monkeypatch):
    ...
    class FakeTable:
        ...
        def execute(self):
            return SimpleNamespace(data=[{"chat_id": "1"}])
...
def test_post_message_forbidden(client, monkeypatch):
    _patch_auth(monkeypatch)
    monkeypatch.setattr(
        res_routes, "get_chat_or_none", lambda *a: {"responder_id": "other-user"}
    )
...
def test_get_chat_detail_success(client, monkeypatch):
    ...
    monkeypatch.setattr(
        res_routes,
        "get_chat_or_none",
        lambda *a: {"status": "open", "responder_id": "user-1"},
    )
...
def test_claim_chat_errors(client, monkeypatch):
    ...
    monkeypatch.setattr(
        res_routes,
        "get_chat_or_none",
        lambda *a: {"status": "claimed", "claim_state": "claimed"},
    )
    resp = client.post("/v1/responder/chats/1/claim", json={"title": "T"})
    ...
    class FakeTableErr:
        ...
        def execute(self):
            raise APIError({"message": self.msg, "code": self.code})
    ...
    monkeypatch.setattr(
        ...
        lambda *a: SimpleNamespace(
            table=lambda *a: FakeTableErr(
                c="new row violates row-level security policy", code="42501"
            )
        ),
    )
    resp = client.post("/v1/responder/chats/1/claim", json={"title": "T"})
    ...
    monkeypatch.setattr(
        res_routes,
        "user_client",
        lambda *a: SimpleNamespace(table=lambda *a: FakeTableErr(code="23505")),
    )
    resp = client.post("/v1/responder/chats/1/claim", json={"title": "T"})
    assert resp.status_code == 409
...
def test_get_chat_detail_errors(client, monkeypatch):
    ...
    monkeypatch.setattr(
        res_routes,
        "get_chat_or_none",
        lambda *a: {"status": "claimed", "responder_id": "other"},
    )
    resp = client.get("/v1/responder/chats/1")
    assert resp.status_code == 403
...
def test_post_message_errors(client, monkeypatch):
    ...
    monkeypatch.setattr(
        res_routes,
        "get_chat_or_none",
        lambda *a: {"responder_id": "user-1", "status": "closing"},
    )
    resp = client.post("/v1/responder/chats/1/messages", json={"message": "M"})
    assert resp.status_code == 400
...
def test_close_chat_errors(client, monkeypatch):
    ...
    monkeypatch.setattr(
        res_routes, "get_chat_or_none", lambda *a: {"responder_id": "other"}
    )
    resp = client.post("/v1/responder/chats/1/close", json={})
    assert resp.status_code == 403
