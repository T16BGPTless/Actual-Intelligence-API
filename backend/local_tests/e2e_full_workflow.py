...
def run_workflow():
    ...
    tokens_res = requests.get(
        f"{BASE_URL}/v1/tokens", headers={"AccessToken": req_token}
    )
    ...
    br_res = requests.get(
        f"{BASE_URL}/v1/responder/chats/unclaimed", headers={"AccessToken": res_token}
    )
    ...
    mal_claim_res = requests.post(
        f"{BASE_URL}/v1/responder/chats/{chat_id}/claim",
        headers={"AccessToken": mal_res_token},
        json={"title": "Steal job"},
    )
    if mal_claim_res.status_code == 200:
        print(
            "SECURITY FAILED: Malicious responder successfully claimed an already claimed chat."
        )
        sys.exit(1)

    # Malicious responder trying to view the claimed chat
    mal_view_res = requests.get(
        f"{BASE_URL}/v1/responder/chats/{chat_id}",
        headers={"AccessToken": mal_res_token},
    )
    if mal_view_res.status_code == 200:
        print(
            "SECURITY FAILED: Malicious responder successfully viewed someone else's claimed chat."
        )
        sys.exit(1)

    # Malicious responder trying to send a message
    mal_msg_res = requests.post(
    ...
    if mal_req_msg.status_code == 201:
        print(
            "SECURITY FAILED: Malicious requester successfully sent a message as another user."
        )
        sys.exit(1)
    ...
    claimed_res = requests.get(
        f"{BASE_URL}/v1/responder/chats", headers={"AccessToken": res_token}
    )
    ...
    req_msg_res = requests.post(
        ...
        headers={"AccessToken": req_token},
        json={
            "message": "Thanks! Could you add a stanza about the moon?",
            "tokensToSpend": 10,
        },
    )
    ...
    requests.post(
        ...
        headers={"AccessToken": res_token},
        json={
            "message": "Sure, here's a moon stanza: The moon glows bright, illuminating the night."
        },
    )
    ...
    res_chat_detail = requests.get(
        f"{BASE_URL}/v1/responder/chats/{chat_id}", headers={"AccessToken": res_token}
    )
    ...
    p(
        "10.5 Malicious Requester creating a legitimate separate chat to add chatter volume"
    )
    mal_req_buy = requests.post(
        f"{BASE_URL}/v1/tokens/buy",
        json={"tokens": 100},
        headers={"AccessToken": mal_req_token},
    )
    ...
    final_chat = requests.get(
        f"{BASE_URL}/v1/requester/chats/{chat_id}", headers={"AccessToken": req_token}
    )
    check_status(final_chat, 200)
    data = final_chat.json()
    print(
        f"Final Chat Status: {data.get('status')} | Claim State: {data.get('claim_state')}"
    )
