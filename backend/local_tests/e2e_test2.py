...
if not req_token:
    req_token = requests.post(
        ...
        json={"email": "reqtest@example.com", "password": "password"},
    ).json()["accessToken"]
...
print(
    "Req tokens:",
    requests.get(f"{BASE_URL}/v1/tokens", headers={"AccessToken": req_token}).json(),
)
...
chat_res = requests.post(
    ...
    json={"requestText": "test req", "category": "writing", "tokensToSpend": 10},
    headers={"AccessToken": req_token},
)
cid = chat_res.json()["chatID"]
print("Created chat", cid)
print(
    "Req tokens after spend:",
    requests.get(f"{BASE_URL}/v1/tokens", headers={"AccessToken": req_token}).json(),
)
...
if not res_token:
    res_token = requests.post(
        f"{BASE_URL}/v1/auth/login",
        json={"email": "restest@example.com", "password": "password"},
    ).json()["accessToken"]
...
print(
    requests.post(
        f"{BASE_URL}/v1/responder/chats/{cid}/close", headers={"AccessToken": res_token}
    ).json()
)
