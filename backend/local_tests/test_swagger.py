...
def user(run_id):
    ...
    res = requests.post(
        f"{BASE_URL}/v1/auth/login", json={"email": email, "password": passw}
    )
...
def test_swagger_auth_register(run_id):
    res = requests.post(
        ...
        json={
            "name": "New",
            "username": f"new_{run_id}",
            "email": f"new_{run_id}@example.com",
...
def test_swagger_auth_login(run_id):
    ...
    requests.post(
        ...
        json={
            "name": "Login Test",
            "username": f"login_test_{run_id}",
            "email": email,
            "password": "password123",
        },
    )
    res = requests.post(
        f"{BASE_URL}/v1/auth/login", json={"email": email, "password": "password123"}
    )
...
def test_swagger_requester_chats_detail(user, chat_id):
    res = requests.get(
        f"{BASE_URL}/v1/requester/chats/{chat_id}", headers={"AccessToken": user}
    )
    assert res.status_code == 200
...
def test_swagger_requester_chats_messages(user, chat_id, run_id):
    ...
    res_claim = requests.post(
        ...
        json={
            "name": "Claimer",
            "username": f"claimer_{run_id}",
            "email": f"claimer_{run_id}@example.com",
            "password": "password123",
        },
    )
    claimer_tok = requests.post(
        f"{BASE_URL}/v1/auth/login",
        json={"email": f"claimer_{run_id}@example.com", "password": "password123"},
    ).json()["accessToken"]

    requests.post(
        f"{BASE_URL}/v1/responder/chats/{chat_id}/claim",
        json={"title": "I claim you"},
        headers={"AccessToken": claimer_tok},
    )

    res = requests.post(
        f"{BASE_URL}/v1/requester/chats/{chat_id}/messages",
        json={"message": "ping", "tokensToSpend": 1},
        headers={"AccessToken": user},
    )
    assert res.status_code == 201
...
def test_swagger_responder_chats_unclaimed(user):
    res = requests.get(
        f"{BASE_URL}/v1/responder/chats/unclaimed", headers={"AccessToken": user}
    )
    assert res.status_code == 200
...
def test_swagger_responder_chats_detail(user, chat_id):
    res = requests.get(
        f"{BASE_URL}/v1/responder/chats/{chat_id}", headers={"AccessToken": user}
    )
